...
public class CheckTrueFalse {
	...
	public static Set<String> hashlist = new HashSet<String>();
	public static HashMap<String, String> model = new HashMap<String, String>();
	public static boolean tail_check = false;
	public static String final_response = "";

	public static int b[][] = new int[16][16];
	public static int m[][] = new int[16][16];
	public static int p[][] = new int[16][16];
	public static int s[][] = new int[16][16];

	public static void main(String[] args) {

		if (args.length != 3) {
			// takes three arguments
			System.out.println("Usage: " + args[0] + " [wumpus_rules-file] [additional-knowledge-file] [input_file]\n");
			exit_function(0);
		}
		...
		// System.out.println("Symbols: " + knowledge_base.getUniqueSymbol());
		arraylist1.addAll(hashlist);
		arraylist2 = arraylist1;
		// System.out.println("Model: " + model);
		// System.out.println("arraylist1: " + arraylist1.get(0));
		boolean tail = tails(knowledge_base, statement, arraylist1, model);
		tail_check = true;
		boolean negative_tail = tails(knowledge_base, statement, arraylist2, model);
		// testing
		if (tail) {
			if (!negative_tail)
				final_response = "definitely true";
			else
				final_response = "both true and false";
		} else {
			if (negative_tail)
				final_response = "definitely false";
			else
				final_response = "possibly true, possibly false";
		}
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter("result.txt"));
			out.write(final_response + "\r\n");
			out.close();
	...
	public static LogicalExpression readExpression(String input_string) {
		...
		// testing
		// System.out.println("readExpression() beginning -" + input_string + "-"); testing
		// System.out.println("\nread_exp");
		...
		if (input_string.startsWith("(")) {
			...
			// testing System.out.println("readExpression() without opening paren -" +symbolString + "-");
	...
	public static Vector<LogicalExpression> read_subexpressions(String input_string) {
		...
		while (input_string.length() > 0) {
			...
			if (input_string.startsWith("(")) {
				...
				// testing System.out.println("read_subexpression() entered if with: ->" + input_string + "<-");
				...
				// readExpression with: ->" + newSymbol + "<- matchingIndex is->" + matchingIndex );

				// pass that string to readExpression,
				newExpression = readExpression(newSymbol);

				// add the LogicalExpression 
				// that it returns to the vector symbolList
				symbolList.add(newExpression);

				// trim the logicalExpression 
				// from the input_string for further processing
				...
				if (input_string.contains(" ")) {
					...
					// testing
					// System.out.println( 
						// "read_subExpression: i just read ->"
				...
				// testing
				// System.out.println( 
					// "readSubExpressions() - trying to add -"
					// + newSymbol + "- as a unique symbol with ->" + input_string +
					// "<- left" 
				// );

				// System.out.println("inside read_subexpressions: " + newSymbol);
				hashlist.add(newSymbol);
				newExpression.setUniqueSymbol(newSymbol);

				// testing
				// System.out.println(
					// "readSubexpression(): added:-" + newSymbol
				...
				// testing
				// System.out.println(
					// "read_subexpression() - after adding: ->"
			...
			// testing
			// System.out.println(
				// "read_subExpression() - left to parse ->" +
	...
	public static boolean valid_expression(LogicalExpression expression) {
		...
		// check for 'if / iff'
		if ((expression.getOperator().equalsIgnoreCase("if")) || (expression.getOperator().equalsIgnoreCase("iff"))) {

			// the connective is either 'if' or 'iff' - so check the number of
			// connectives
			if (expression.getSubexpressions().size() != 2) {
				System.out.println("error: connective \"" + expression.getOperator() + "\" with " + expression.getSubexpressions().size() + " arguments\n");
				return false;
			}
		}
		// end 'if / iff' check
		...
		// check for 'and / or / xor'
			...
			(!expression.getOperator().equalsIgnoreCase("and"))
	...
	private static boolean tails(LogicalExpression kb, LogicalExpression alpha, ArrayList<String> symbol, HashMap<String, String> model) {
		return checkAll(kb, alpha, symbol, model);
	}

	private static boolean checkAll(LogicalExpression k_base, LogicalExpression alp, ArrayList<String> symbol, HashMap<String, String> model1) 
	{
		// System.out.println(model);
		if (symbol.isEmpty()) {
			if (PLCheck(k_base, model1, false)) {
				return PLCheck(alp, model, !tail_check);
			} else {
				return true;
			}
		} else {
			...
			return checkAll(k_base, alp, rest, EXTEND(p, true, model))
					&& checkAll(k_base, alp, rest, EXTEND(p, false, model));
		}
	}

	private static boolean PLCheck(LogicalExpression know_base, HashMap<String, String> mode, boolean tail_check) {
		boolean check = know_base.getSolution(model);
		if (tail_check) {
			return check;
		} else {
			return !check;
	...
	public static boolean getResponseFromArr(String symbol) {
		...
		if (start_sym.equals("B")) {
			if (b[x][y] == 1) {
				return true;
			} else {
				return false;
			}
		} else if (start_sym.equals("M")) {
			if (m[x][y] == 1) {
				return true;
			} else {
				return false;
